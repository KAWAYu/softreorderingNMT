...
class ReorderingEncoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size):
        ...
        self.forward_gru = nn.GRU(hidden_size, hidden_size, num_layers=2, bidirectional=True)
...
class AttentionDecoder(nn.Module):
    ...
    def forward(self, xs, hidden, ehs):
        ...
        attention = attention_weight / torch.sum(attention_weight)
        context_vector = torch.sum(attention.unsqueeze(1).expand_as(ehs) * ehs, dim=0)
        output = torch.tanh(self.c2h(torch.cat((h.squeeze(1), context_vector), dim=1)))
